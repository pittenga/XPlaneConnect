...
class XPlaneConnect(object):
    ...
    def selectDATA(self, data):
        '''Selects X-Plane data over the underlying UDP socket.

            Args:
              data: An array of values representing data rows to be selected.
        '''
        if len(data) > 134:
            raise ValueError("Too many rows in data.")

        buffer = struct.pack("<4sx", "DSEL")
        for item in data:
            if item > 255:
                raise ValueError("Select DATA item " + item + " out of bounds.")
